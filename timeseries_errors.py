...
def _area_error(y, y_hat, score_window=10):
    ...
    smooth_y = pd.Series(y).rolling(
        score_window, center=True, min_periods=score_window // 2).apply(np.trapz)
...
def _cosine_similarity(seq1, seq2):
    """
    計算兩條序列的餘弦相似度
    Args:
        seq1, seq2: shape = [T] or [T, D]，長度一致
    Returns:
        相似度值 (float)，越接近 1 越相似
    """
    seq1 = np.ravel(seq1)
    seq2 = np.ravel(seq2)
    numerator = np.dot(seq1, seq2)
    denominator = np.linalg.norm(seq1) * np.linalg.norm(seq2)
    if denominator == 0:
        return 0.0
    return numerator / denominator


def _cosine_similarity_series(y, y_hat, window=10):
    """
    用滑動視窗方式計算每個時間點的餘弦誤差（1 - similarity）
    Args:
        y: Ground truth (1D)
        y_hat: Prediction (1D)
        window: Window size for local similarity
    Returns:
        errors: 每個點的餘弦誤差值
    """
    half_w = window // 2
    padded_y = np.pad(y, (half_w, half_w), mode='reflect')
    padded_y_hat = np.pad(y_hat, (half_w, half_w), mode='reflect')

    errors = []
    for i in range(len(y)):
        seg_y = padded_y[i:i+window]
        seg_y_hat = padded_y_hat[i:i+window]
        sim = _cosine_similarity(seg_y, seg_y_hat)
        errors.append(1 - sim)  # 越小越相似
    return np.array(errors)


def _pearson_correlation_series(y, y_hat, window=10):
    half_w = window // 2
    padded_y = np.pad(y, (half_w, half_w), mode='reflect')
    padded_y_hat = np.pad(y_hat, (half_w, half_w), mode='reflect')
    ...
    for i in range(len(y)):
        seg_y = padded_y[i:i+window]
        seg_y_hat = padded_y_hat[i:i+window]
        sim = _pearson_correlation(seg_y, seg_y_hat)
        errors.append(1 - sim)  # 越小越相似
    return np.array(errors)

def _pearson_correlation(seq1, seq2):
    """
    計算兩條序列的皮爾森相關係數
    Args:
        seq1, seq2: shape = [T] or [T, D]，長度一致
    ...
    seq1 = np.ravel(seq1)
    seq2 = np.ravel(seq2)
    if np.std(seq1) < 1e-8 or np.std(seq2) < 1e-8:
        return 0.0
...
def _discrete_frechet_distance(P, Q):
    """
    離散 Frechet 距離，適用於時間序列的軌跡比較。
    Args:
        P, Q: shape = [T, D] 或 [T]，時間序列
    Returns:
        Frechet 距離 (float)
    """
    P = np.atleast_2d(P)
...
def _frechet_distance_series(y, y_hat, window=10):
    half_w = window // 2
    padded_y = np.pad(y, (half_w, half_w), mode='reflect')
    padded_y_hat = np.pad(y_hat, (half_w, half_w), mode='reflect')
    ...
    for i in range(len(y)):
        seg_y = padded_y[i:i+window]
        seg_y_hat = padded_y_hat[i:i+window]
        dist = _discrete_frechet_distance(seg_y, seg_y_hat)
        errors.append(dist)
    return np.array(errors)
...
def dtw_errors(true, predictions, score_window=10, smoothing_window=0.01, smooth=False,
               rec_error_type='point'):

    if isinstance(smoothing_window, float):
        smoothing_window = min(math.trunc(len(true) * smoothing_window), 200)
    ...
    elif rec_error_type.lower() == "cosine":
        errors = _cosine_similarity_series(true, predictions)
        
    elif rec_error_type.lower() == "pearson":
        errors = _pearson_correlation_series(true, predictions)
        
    elif rec_error_type.lower() == "discrete":
        errors = _frechet_distance_series(true, predictions)
    
    # Apply smoothing
    if smooth:
        errors_avg = pd.Series(errors).rolling(
            smoothing_window, center=True, min_periods=smoothing_window // 2).mean().values

    return pd.Series(errors_avg), pd.Series(errors)